...
/*! @def _rMessageDef
  ...
  @internal
*/
#define _rMessageDef(ID, COMPONENT) static rlog::PublishLoc ID RLOG_SECTION = {& ID ## _enabled, &rlog::RLog_Register, 0, STR(COMPONENT), __FILE__, __FUNCTION__, __LINE__, 0};
...
/*! @def _rMessage(ID, CHANNEL, ... )
  ...
  REMOVED do{}while(0) - it's a hideous and improper method of achieving the stated aim!
  @internal
*/
#define _rMessage(ID, CHANNEL, ... ) { _rMessageCall(ID, RLOG_COMPONENT, CHANNEL, ##__VA_ARGS__ ) }
...
#define rIWA(...) \
  _rMessage( LOGID, rlog::_RLIWAChannel, ##__VA_ARGS__ )


#define rSync(...) \
  _rMessage( LOGID, rlog::_RLSyncChannel, ##__VA_ARGS__ )
