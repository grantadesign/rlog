...
void RLogPublisher::PublishVA(PublishLoc *loc, RLogChannel *, const RLOG_CHAR_T *format, va_list ap)
{
    RLogData data;
    ...
    RLOG_CHAR_T msgBuf[64];
    RLOG_CHAR_T *buf = msgBuf;
    size_t bufSizeInChars = sizeof(msgBuf) / sizeof(RLOG_CHAR_T);
    ...
    for(int numTries = 10; numTries; --numTries)
    {
	    ...
	    int ncpy = RLOG_VPRINTF( buf , bufSizeInChars, _TRUNCATE, format, args );
	    va_end( args );

	    // if it worked, then return the buffer
	    if( ncpy > -1 && ncpy < (int)bufSizeInChars )
	    {
	        data.msg = buf;
	        break;
	    } 
        else
	    {
	        ...
	        if(ncpy > 0) 
		        bufSizeInChars = ncpy + 1;
	        else
		        bufSizeInChars *= 2; // try twice as much space as before

	        if(buf != msgBuf)
		        delete[] buf;

	        buf = new RLOG_CHAR_T[bufSizeInChars];
	    }
    }
